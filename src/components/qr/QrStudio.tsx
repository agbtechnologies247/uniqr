...
const MARGIN_OPTIONS: { value: number; label: string; desc: string }[] = [
  { value: 1, label: "Tight", desc: "1 module (tight label)" },
  { value: 2, label: "Std", desc: "2 modules (standard)" },
  { value: 4, label: "Wide", desc: "4 modules (ISO print)" },
];

const MODULE_STYLES: { id: QrStyleType; label: string; desc: string }[] = [
  { id: "rounded-modules", label: "Rounded", desc: "Smooth rounded blocks" },
  { id: "circular-dots", label: "Dots", desc: "Circular micro-dots" },
  { id: "soft-rounded", label: "Soft", desc: "Subtle softened corners" },
  { id: "square", label: "Square", desc: "Classic crisp matrix" },
];

const ENCODING_OPTIONS: { mode: EncodingMode; label: string; chars: string }[] = [
  { mode: "auto", label: "Auto", chars: "All characters" },
  { mode: "numeric", label: "Numeric", chars: "0-9 only (3x density)" },
...
export const QrStudio: React.FC<QrStudioProps> = ({
  ...
  // Live Reactive QR Preview Generation
  useEffect(() => {
    let isMounted = true;
    generateQrDataUrlAsync(qrCodeValue, {
      size: Math.min(downloadSize, 800),
  ...
  const getVisualDimension = () => {
    if (downloadSize <= 236) return "60%";
    if (downloadSize <= 354) return "75%";
    if (downloadSize <= 590) return "90%";
    if (downloadSize <= 1772) return "96%";
    return "100%";
  };
    ...
    <div className="selection:bg-[#1D4533] selection:text-[#F7EAE0]">
      ...
      <div className="lg:hidden grid grid-cols-2 gap-2 px-2.5 pb-28">
        ...
        <div className="bg-white rounded-2xl border border-[#F9D2BA] overflow-hidden shadow-sm flex flex-col">
          <div className="bg-[#1D4533] px-2.5 py-1.5 flex items-center justify-between">
            ...
            <span className="text-[9px] font-mono font-bold text-[#F9D2BA]">{downloadSize}px</span>
          </div>

          <div className="flex-1 bg-[#F7EAE0] p-2 flex items-center justify-center min-h-[140px]">
            <div className="w-full aspect-square max-w-[130px] mx-auto flex items-center justify-center">
              {previewDataUrl ? (
                <img
                  src={previewDataUrl}
                  alt="QR Code"
                  className="w-full h-full object-contain rounded-xl border-2 border-[#1D4533] shadow-md bg-white block"
                />
              ) : (
                <div className="w-full h-full flex items-center justify-center">
                  <RefreshCw className="w-5 h-5 animate-spin text-[#1D4533]" />
                </div>
              )}
            </div>
          </div>
          ...
          <div className="px-2 pt-1 pb-1 flex items-center justify-between">
            ...
            <span className="text-[9px] font-bold text-[#1D4533] capitalize">{styleConfig.cornerDotStyle}</span>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-[#F9D2BA] overflow-hidden shadow-sm flex flex-col">
          ...
          <div className="flex-1 p-2 space-y-2 overflow-y-auto" style={{ maxHeight: 280 }}>
            {/* Theme Presets */}
            <div>
              <p className="text-[9px] font-extrabold text-[#5E3122] uppercase tracking-wider mb-1">Theme</p>
              <div className="grid grid-cols-2 gap-1">
                {THEMES.map(t => (
                  <button key={t.name} type="button"
                    onClick={() => { sound.playClick(); setStyleConfig(p => ({ ...p, fgColor: t.fg, bgColor: t.bg, transparentBg: false })); }}
                    className={`p-1.5 rounded-lg border flex items-center gap-1.5 transition-all ${styleConfig.fgColor === t.fg && styleConfig.bgColor === t.bg && !styleConfig.transparentBg ? "bg-[#1D4533] text-[#F7EAE0] border-[#1D4533]" : "bg-[#F7EAE0]/60 border-[#F9D2BA] text-[#5E3122]"}`}
                  >
            ...
            <div>
              <p className="text-[9px] font-extrabold text-[#5E3122] uppercase mb-1">Colors</p>
              <div className="flex gap-2">
                <label className="flex items-center gap-1 text-[9px] font-bold text-[#1D4533] cursor-pointer">
                  <input type="color" value={styleConfig.fgColor} onChange={e => setStyleConfig(p => ({ ...p, fgColor: e.target.value }))} className="w-6 h-5 rounded border-0 p-0 cursor-pointer" />
                  <span>QR</span>
                </label>
                <label className="flex items-center gap-1 text-[9px] font-bold text-[#1D4533] cursor-pointer">
                  <input type="color" value={styleConfig.bgColor} onChange={e => setStyleConfig(p => ({ ...p, bgColor: e.target.value }))} className="w-6 h-5 rounded border-0 p-0 cursor-pointer" />
                  <span>BG</span>
                </label>
              </div>
            </div>
            ...
            {/* Module Style */}
            <div>
              <p className="text-[9px] font-extrabold text-[#5E3122] uppercase mb-1">Module Shape</p>
              <div className="grid grid-cols-2 gap-1">
                {MODULE_STYLES.map(ms => (
                  <button key={ms.id} type="button"
                    onClick={() => { sound.playClick(); setStyleConfig(p => ({ ...p, style: ms.id })); }}
                    className={`py-1 px-1 rounded-lg border text-center transition-all ${styleConfig.style === ms.id ? "bg-[#1D4533] text-[#F7EAE0] border-[#1D4533]" : "bg-[#F7EAE0] border-[#F9D2BA] text-[#5E3122]"}`}
                  >
                    <span className="text-[9px] font-bold block">{ms.label}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Finder Pattern */}
            <div>
              <p className="text-[9px] font-extrabold text-[#5E3122] uppercase mb-1">Finder Pattern</p>
        ...
        <div className="bg-white rounded-2xl border border-[#F9D2BA] overflow-hidden shadow-sm flex flex-col">
          ...
          <div className="flex-1 p-2 space-y-2 overflow-y-auto" style={{ maxHeight: 360 }}>
            {/* Raster */}
            <div>
              <p className="text-[9px] font-extrabold text-[#5E3122] uppercase mb-1">Raster</p>
              <div className="flex gap-1">
            ...
            {/* Vector */}
            <div>
              <p className="text-[9px] font-extrabold text-[#5E3122] uppercase mb-1">Vector / Print</p>
              <div className="grid grid-cols-2 gap-1">
            ...
            {/* CAD */}
            <div>
              <p className="text-[9px] font-extrabold text-[#5E3122] uppercase mb-1">CAD / CNC</p>
            ...
            <div className="p-1.5 bg-[#F7EAE0] rounded-lg border border-[#F9D2BA]">
              <p className="text-[9px] font-extrabold text-[#1D4533]">Format: {selectedFormat.toUpperCase()}</p>
              ...
              <div style={{ width: vd, height: vd }} className="aspect-square transition-all duration-300 flex items-center justify-center">
                {previewDataUrl ? (
                  <img
                    src={previewDataUrl}
                    alt="QR Code Preview"
                    className="w-full h-full object-contain rounded-2xl border-2 border-[#1D4533] shadow-md bg-white block select-none"
                  />
                ) : (
                  <div className="w-full h-full flex items-center justify-center">
                    <RefreshCw className="w-8 h-8 animate-spin text-[#1D4533]" />
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="bg-white p-8 rounded-3xl border border-[#F9D2BA] space-y-4 shadow-sm">
            ...
            <div className="grid grid-cols-4 gap-2.5">
              {THEMES.map(t => (
                <button key={t.name} type="button"
                  onClick={() => { sound.playClick(); setStyleConfig(p => ({ ...p, fgColor: t.fg, bgColor: t.bg, transparentBg: false })); }}
                  className={`p-2.5 rounded-2xl border text-left transition-all flex items-center gap-2 ${styleConfig.fgColor === t.fg && styleConfig.bgColor === t.bg && !styleConfig.transparentBg ? "bg-[#1D4533] text-[#F7EAE0] border-[#1D4533] shadow-sm" : "bg-[#F7EAE0]/50 border-[#F9D2BA] text-[#5E3122] hover:bg-white"}`}>
            ...
            <div className="flex flex-wrap items-center gap-4 pt-1">
              ...
              <label className="flex items-center gap-2 cursor-pointer text-xs font-bold text-[#5E3122]">
                <input type="checkbox" checked={styleConfig.gradient}
                  onChange={e => { sound.playClick(); setStyleConfig(p => ({ ...p, gradient: e.target.checked })); }}
                  className="rounded border-[#F9D2BA] text-[#1D4533]" />
                <span>Gradient</span>
              </label>
              {styleConfig.gradient && (
                <label className="flex items-center gap-1.5 text-xs font-bold text-[#5E3122] cursor-pointer">
                  <span>Grad Color:</span>
                  <input type="color" value={styleConfig.gradientColor || "#5E3122"} onChange={e => setStyleConfig(p => ({ ...p, gradientColor: e.target.value }))} className="w-7 h-6 rounded border-0 p-0 cursor-pointer" />
                </label>
              )}
            </div>

            {/* Module shapes */}
            <div className="pt-2 border-t border-[#F9D2BA]">
              <span className="text-xs font-extrabold text-[#1D4533] block mb-2">Module Geometry Shape</span>
              <div className="grid grid-cols-4 gap-2">
                {MODULE_STYLES.map(ms => (
                  <button key={ms.id} type="button"
                    onClick={() => { sound.playClick(); setStyleConfig(p => ({ ...p, style: ms.id })); }}
                    className={`p-2.5 rounded-2xl border text-left transition-all ${styleConfig.style === ms.id ? "bg-[#1D4533] text-[#F7EAE0] border-[#1D4533] shadow-sm" : "bg-[#F7EAE0]/50 border-[#F9D2BA] text-[#5E3122] hover:bg-white"}`}
                  >
                    <span className="font-extrabold text-xs block">{ms.label}</span>
                    <span className="text-[9px] opacity-80 block truncate">{ms.desc}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white p-8 rounded-3xl border border-[#F9D2BA] space-y-5 shadow-sm">
            ...
            <div className="space-y-2">
              <label className="text-xs font-extrabold text-[#1D4533] flex items-center gap-1.5">
                ...
                Finder Pattern Style (Corner Detection Squares)
              </label>
              <p className="text-[10px] text-[#5E3122] font-semibold -mt-1">
                The 3 large corner squares used by camera sensors to detect orientation, size, and angle.
              </p>
              <div className="flex gap-2">
                {(["square", "rounded", "dots"] as const).map(s => (
                  <button key={s} type="button" onClick={() => { sound.playClick(); setStyleConfig(p => ({ ...p, cornerDotStyle: s })); }}
                    ...
                    <span className="font-extrabold text-xs capitalize">{s} Pattern</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-[#5E3122] font-semibold">
                  Blank border ensuring scanners isolate the code from surrounding surfaces.
                </p>
                <div className="flex gap-2">
                  {MARGIN_OPTIONS.map(m => (
                    <button key={m.value} type="button"
              ...
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-[#5E3122] font-semibold">
                  Character set. Numeric encodes 3× more data per module than binary.
                </p>
